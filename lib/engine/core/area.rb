...
module Engine
  class Area
    ...
    def initialize(top_point, width, height)
      ...
      if(top_point.nil? || !top_point.is_a?(Engine::Point))
        raise ArgumentError.new "Top point can only be a non-nil instance of #{Engine::Point}"
      end
    ...
    def top_point=(value)
      if(value.nil? || !value.is_a?(Engine::Point))
        raise ArgumentError.new "Top point can only be a non-nil instance of #{Engine::Point}"
      end
