...
module Engine
  module Scene
    ...
    def add_control(new_control)
      if(new_control.nil? || !new_control.is_a?(Engine::Control))
        raise ArgumentError.new "New control must be a non-nil descendant of #{Engine::Control}"
      end
    ...
    def button_up(id, point)
      if(point.nil? || !point.is_a?(Engine::Point))
        raise ArgumentError.new "Argument can only be a non-nil instance of #{Engine::Point}"
      end
    ...
    def update(point)
      if(@prev_mouse_position.nil? || @prev_mouse_position != point)
        @prev_mouse_position = Engine::Point.new(point.x, point.y)
